...
// Define slot symbols with their probabilities (higher number = higher chance)
const SYMBOLS = [
  { id: 0, symbol: '🍒', label: 'Cherry', probability: 20, value: 1 },
  { id: 1, symbol: '🍋', label: 'Lemon', probability: 17, value: 2 },
  { id: 2, symbol: '🍊', label: 'Orange', probability: 15, value: 3 },
  { id: 3, symbol: '🍇', label: 'Grape', probability: 12, value: 5 },
  { id: 4, symbol: '🍉', label: 'Watermelon', probability: 10, value: 8 },
  { id: 5, symbol: '🔔', label: 'Bell', probability: 8, value: 10 },
  { id: 6, symbol: '💎', label: 'Diamond', probability: 5, value: 20 },
  { id: 7, symbol: '7️⃣', label: 'Seven', probability: 3, value: 50 },
];
    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-6 text-center">
        ...
        <p className="text-muted-foreground">
          Try your luck with this simple slot machine
        </p>
      </div>

      <Card className="shadow-lg overflow-hidden relative bg-gradient-to-b from-gray-900 to-gray-800 text-white border-none">
        <CardHeader className="bg-gradient-to-r from-purple-700 to-blue-700 pb-4 pt-6">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              <CardTitle className="text-2xl text-white">Slot Machine</CardTitle>
            </div>
        ...
        <CardContent className="p-6">
          ...
          {/* Slot machine */}
          <div className="bg-gradient-to-b from-gray-700 to-gray-800 p-4 rounded-lg shadow-inner border border-gray-600 mb-5 overflow-hidden">
            {/* Reels container */}
            <div className="flex justify-between gap-2">
              {reels.map((reel, reelIndex) => (
                <div 
                  key={reelIndex}
                  className="flex-1 bg-white dark:bg-gray-900 rounded-md overflow-hidden shadow-inner border-2 border-amber-500"
                >
                    ...
                    {reel.map((symbol, symbolIndex) => (
                      <motion.div
                        ...
                        className={`flex items-center justify-center w-full py-5 text-4xl ${
                          symbolIndex === 1 ? "bg-amber-100 dark:bg-amber-900/30" : ""
                        }`}
                        animate={isSpinning ? {
                          y: [0, -20, 0],
                          transition: {
          ...
          {showWin && (
            <motion.div 
              className="bg-green-600 text-white text-center p-3 rounded-md mb-4 font-bold text-lg"
      ...
      <div className="mt-4 text-sm text-center text-muted-foreground">
        ...
        <p>Match symbols on the middle payline to win!</p>
      </div>
    </div>
  );
}
